...
- **桌面布局**：工具集中在编辑区上方；AI、资料与版本共用工具侧栏，同一时刻只显示一类面板。按容器实际宽度保留至少 600px 编辑空间，不足时临时收起目录，工具关闭或空间恢复后还原目录偏好。目录宽度 180–320px、工具宽度 280–520px，可拖动或键盘调整并记忆；更窄的视口按现有单栏逻辑呈现。正文默认宽度上限为 820px，保留作者已有宽度设置。
- **资料随写随查**：写作页可搜索人物与备忘、查看作品设定、快速记录备忘；关闭资料面板保留草稿，草稿按用户和作品隔离。
...
- **生成审阅**：标明作用范围、所用模型与保存行为；开启预览后可对比原文、按段落采纳，标题与概要单独选择。取消恢复原稿，确认前不自动保存候选内容。
- **新建作品**：创建后进入空白第一章，允许直接手写；作品设定是按需补充资料。作品库展示字数、章节数、最近编辑章节与时间，提供网格和紧凑列表。
- **AI 设置**：按模型连接、生成偏好、高级选项分组；模型切换立即保存，其他分组显式保存，切换分组或模型不丢弃未保存的偏好。
- **移动导航**：长书名截断；账户菜单保留语言、主题和全局页面入口；常用写作按钮触控区不小于 44px。
- **作品导航**：工作区顺序为写作、设定、人物、备忘；书名可切换作品，返回按钮注明目的地。切换作品也经过草稿离开保护。
- **作品整理**：最近继续采用紧凑条目；作品可置顶与归档，归档可恢复，不删除内容，整理操作不改变最近编辑时间。
- **管理页面**：标题、搜索与主要操作采用紧凑布局；删除与导出归入更多菜单，不与继续写作争夺视觉重点。
- **作品资料页面**：设定、人物、备忘共用 1080px 内容宽度与独立页标题；表单采用分组说明和字段两列排布，保留 24–32px 内边距。列表提供搜索、摘要与就近添加入口，空状态不强撑高度；助手入口置于页标题区，避免遮挡内容。样式限定在资料页面类名内，不通过嵌套 Ant Card 选择器覆盖通用内边距。
- **资料阅读与编辑**：宽屏采用列表与只读详情并排，窄屏上下浏览；明确点击编辑后进入表单。设定、人物、备忘显示未保存状态，离开前提供保存、放弃与继续编辑，保存失败保留草稿，切换语言不覆盖输入。
- **辅助页面**：AI 设置提供不生成正文的连接检查，区分未配置、检查通过、失败与接口不支持；任务可筛选状态，优先展示进行中和失败任务；用量先展示累计输入、输出、总量与来源，再列近期明细，配额另行展示。刷新采用次级按钮。
